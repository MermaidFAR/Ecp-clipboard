...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let _hotkey_thread = spawn_hotkey_listener(command_tx.clone(), config.use_win_v_hotkey);
    let _tray_icon = create_tray(command_tx.clone())?;
...
fn create_tray(command_tx: mpsc::Sender<UiCommand>) -> Result<TrayIcon, Box<dyn Error>> {
    let menu = Menu::new();
    ...
    thread::spawn(move || {
        ...
        while let Ok(event) = receiver.recv() {
            if event.id == show_id {
                ...
                let _ = command_tx.send(UiCommand::Exit);
                break;
            }
        }
    });
...
fn spawn_hotkey_listener(
    ...
    thread::spawn(move || unsafe {
        if let Err(error) =
            RegisterHotKey(None, CTRL_ALT_V_ID, MOD_CONTROL | MOD_ALT, VK_V.0 as u32)
        {
        ...
        if use_win_v_hotkey {
            match RegisterHotKey(None, WIN_V_ID, MOD_WIN, VK_V.0 as u32) {
                Ok(()) => {
                    let _ =
                        command_tx.send(UiCommand::Status(String::from("Win+V 替代热键已启用")));
                }
                Err(error) => {
                    let _ = command_tx.send(UiCommand::Status(format!(
                        "Win+V 注册失败，可能被系统占用: {error}"
                    )));
                }
            }
        }
